...
GAMEDIR=/$directory/ports/frontier/
...
$GPTOKEYB2 "frontier.${DEVICE_ARCH}" -c "./frontier.ini" &

# Do some platform specific stuff right before the port is launched but after GPTOKEYB is run.
pm_platform_helper "$GAMEDIR/frontier.${DEVICE_ARCH}"
...
# Frontier specific: Setup sdl12-compat options:
export SDL12COMPAT_SCALE_METHOD=nearest # Prevent ugly blurriness
export SDL12COMPAT_WINDOW_SCALING=2     # double resolution from 320x200 to 640x400
...
./frontier.${DEVICE_ARCH} -f # Launch the executable
